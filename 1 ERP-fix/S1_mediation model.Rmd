---
title: "ERP-fix: Multilevel mediation model"
author: "Hannah"
date: "4/20/2020"
...
Incorrect trials and trials with +/- 75 uV artifacts are excluded. Subjects with < 50% trials accepted are excluded (subs 32, 51 and 62).
...
b1 = lmer(RT ~ P2.mean+P2.meancent + (1|Subject), data = dat) 
...
```{r}
# just include within and between
b2 = lmer(PeakLat ~ P2.mean+P2.meancent + (1|Subject), data = dat) 
# Electrode is not included as random variable because everything has been centered by electrode 
# (i.e. it doesn't contribute any variance)
...
full = lmer(PeakLat ~ P2.mean+P2.meancent + (1|Subject), data = dat)  %>% r.squaredGLMM()
reduced = lmer(PeakLat ~ P2.mean + (1|Subject), data = dat)  %>% r.squaredGLMM() 
...
```{r}
# just include within and between
b3 = lmer(RT ~ P2.mean+P3lat.mean+P2.meancent+P3lat.meancent + (1|Subject), data = dat)
# Electrode is not included as random variable because everything has been centered by electrode 
# (i.e. it doesn't contribute any variance)
